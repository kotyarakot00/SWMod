(function(container, showMessage) {
    ...
    function getAvatarPart(partName, callback) {
        window.waitForElement(`.avatar__part_${partName}`, callback);
    }
    
    container.innerHTML = '<div style="text-align:center; padding:20px;">⏳ Загрузка персонажа...</div>';
    
    getAvatarPart('clothes', (clothesPart) => {
        if(!clothesPart) {
            container.innerHTML = '<div style="color:#ff6170; text-align:center; padding:20px;">❌ Персонаж не найден</div>';
            return;
        }
        ...
        document.getElementById('sw-apply-clothes').addEventListener('click', () => {
            ...
            window.waitForElement('.avatar__part_clothes', (clothesPart) => {
                if(value === '0') {
        ...
        document.getElementById('sw-apply-hair').addEventListener('click', () => {
            ...
            window.waitForElement('.avatar__part_hair-front', (hairPart) => {
                if(value === '0') {
                    hairPart.innerHTML = '';
